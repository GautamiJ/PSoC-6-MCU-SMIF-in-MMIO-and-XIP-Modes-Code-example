...
cy_en_smif_status_t EnableQuadMode(cy_stc_smif_mem_config_t const *memConfig)
{
	...
	status = Cy_SMIF_MemEnableQuadMode(KIT_QSPI_HW, smifMemConfigs[0], 5000,
			&KIT_QSPI_context);
...
int main(void)
{
    ...
    /* \x1b[2J\x1b[;H - ANSI ESC sequence for clear screen */
        ...
        smifStatus = Cy_SMIF_MemIsQuadEnabled(KIT_QSPI_HW, smifMemConfigs[0], &isQuadEnabled, &KIT_QSPI_context);

    	if((CY_SMIF_SUCCESS == smifStatus) && !isQuadEnabled)
    	{
    		printf("[Info] Quad not Enabled for External Flash 1\r\n\r\n");
    		smifStatus = EnableQuadMode(smifMemConfigs[0]);
    		printf("[Info] Quad Enabled successfully\r\n\r\n");
    	}
